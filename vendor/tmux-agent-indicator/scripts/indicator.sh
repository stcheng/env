...
codex_rate_limits() {
    local script="$PLUGIN_DIR/scripts/codex-rate-limits.sh"
...
output_indicator() {
    ...
    if [ "$state" = "running" ] && is_animation_enabled; then
        ...
        if [ -n "$frame" ]; then
            print_indicator "$(render_knight_rider "$frame") $icon" "$agent"
            return
        fi
    fi
    print_indicator "$icon" "$agent"
}
...
if [ -n "$PANE_TTY" ]; then
    ...
    for proc in "${PROC_ARRAY[@]}"; do
        ...
        if ps -t "$(basename "$PANE_TTY")" -o command= 2>/dev/null | grep -qw "$proc"; then
            print_indicator "$(icon_for_agent "$proc" "$ICONS")" "$proc"
            exit 0
        fi
    done
fi
